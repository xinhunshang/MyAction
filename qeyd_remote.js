...
async function changeFiele(content, cookies) {
    //替换各种信息.
    for (var i = 0; i < Cookies.length; i++) {
       // console.log(`正在执行第${i + 1}个账号`);
      qqreadBD.push(Cookies[i].split("@")[0]);
      qqreadtimeURL.push(Cookies[i].split("@")[1]);
      qqreadtimeHD.push(Cookies[i].split("@")[2]);
    }
    ...
    content = content.replace("let qqreadBD = [];", `let qqreadBD = qqreadBD;`)
    content = content.replace("let qqreadtimeURL = [];", `let qqreadtimeURL = qqreadtimeURL;`)
    content = content.replace("let qqreadtimeHD = [];", `let qqreadtimeHD = qqreadtimeHD;`)
...
async function start() {
    ...
    await changeFiele(content, Cookies);
    console.log("替换变量完毕");
    // 执行代码
